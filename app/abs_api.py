...
class ABSClient:
    ...
    def trigger_library_scan(self, library_id: str) -> Tuple[bool, str]:
        """Trigger a full library rescan. Returns (success, message)."""
    ...
    def scan_item(self, item_id: str) -> Tuple[bool, str]:
        """
        Trigger a targeted rescan of a single library item.
        Much faster than a full library scan and gives ABS the best chance
        of matching the moved file by inode rather than creating a duplicate.
        Returns (success, message).
        """
        url = f"{self.base_url}/api/items/{item_id}/scan"
        try:
            resp = self.session.post(url, timeout=self.timeout)
            if resp.status_code in (200, 204):
                return True, "Item scan triggered."
    ...
    def update_item_metadata(
        ...
        """
        Patch metadata for a single library item via the ABS API.
        Only fields that are not None are sent. ABS merges the patch.
        Returns (success, message).
        """
        ...
        if author is not None:
            patch["metadata"]["authors"] = [{"name": author}]

        # Series — ABS expects the full series array
        if series_name is not None or series_sequence is not None:
            series_entry: Dict[str, Any] = {}
            if series_name is not None:
                series_entry["name"] = series_name
            if series_sequence is not None:
                series_entry["sequence"] = series_sequence
            patch["metadata"]["series"] = [series_entry]

        if not patch["metadata"]:
            return True, "No changes."

        try:
            resp = self.session.patch(url, json=patch, timeout=self.timeout)
            if resp.status_code in (200, 204):
